...
Nodo *pasajeDeNodos(Nodo **listaI, Nodo **listaF, int *incremento)
{
    Nodo **aux = listaI;
...
void consultaTareas(Nodo **listaI, Nodo **listaF, int *consulta)
{
    char palabraClave[C];
    int idconsulta;
    ...
    {
    case 1:
        printf("Ingrese el id de la tarea para buscar en las tareas: \n");
        scanf("%d", &idconsulta);
        while (aux1 || axu2)
        {

            if (aux1 != NULL && aux1->T.TareaID == idconsulta)
            {
                printf("La tarea pertenece a tarea pendiente");
                break;
            }
            else if (axu2 != NULL && axu2->T.TareaID == idconsulta)
            {
...
int main()
{
    ...
    while (PasajeDeNodo == 1)
    {
        ...
        Nodo *tareaRealizada = pasajeDeNodos(&TareasPendientes, &TareasRealizadas, &idTarea);
        insertarNodo(&TareasRealizadas, tareaRealizada);
    ...
    while (consulta < 3 && consulta > 0)
    {
        ...
        consultaTareas(&TareasPendientes, &TareasRealizadas, &consulta);
